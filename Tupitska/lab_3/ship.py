...
@dataclass(kw_only=True)
class LightWeightShip(BaseShip):
    ship_counter: int = 1

    def get_next_id(self) -> int:
        return LightWeightShip.ship_counter

@dataclass(kw_only=True)
class MediumShip(BaseShip):
    ship_counter: int = 1

    def get_next_id(self) -> int:
        return MediumShip.ship_counter

@dataclass(kw_only=True)
class HeavyShip(BaseShip):
    ship_counter: int = 1

    def get_next_id(self) -> int:
        return HeavyShip.ship_counter
